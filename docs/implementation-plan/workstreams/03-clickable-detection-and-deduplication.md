...
- [ ] Replace role-only clickability with capability-based detection and preserve distinct nested targets during deduplication.
...
- [ ] Inventory the existing `clickableRoles` and `skipSubtreeRoles` usage.
- [ ] Define a capability-first notion of clickability using available AX actions, enabled state, focusability, or settable attributes.
- [ ] Remove unconditional `AXStaticText` clickability unless a concrete actionable rule justifies it.
- [ ] Ensure subtree pruning does not conflict with roles that can still contain actionable descendants.
- [ ] Refine deduplication so child targets with distinct frames or distinct action surfaces survive.
- [ ] Preserve performance by keeping batch-fetch patterns where possible.
- [ ] Keep the public behavior of `HintModeController` unchanged unless absolutely necessary.
